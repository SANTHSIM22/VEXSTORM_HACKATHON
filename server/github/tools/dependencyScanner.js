...
/**
 * DEPENDENCY SCANNER — standalone plain-JS (no LangChain)
 * Extracted from extension/tools/dependencyTools.js
 */
...
const SUSPICIOUS_PATTERNS = ['serialize', 'unserialize', 'eval-', 'exec-', 'shell-'];

function auditPackageJson(content, filePath) {
  let pkg;
  try { pkg = JSON.parse(content); } catch (e) { return { error: `Invalid JSON in ${filePath}: ${e.message}` }; }
  ...
  for (const [name, versionSpec] of Object.entries(allDeps || {})) {
    ...
    if (info) {
      vulnerabilities.push({ package: name, installedVersion: versionSpec, severity: info.severity, cve: info.cve, cwe: info.cwe, description: info.description, fixedIn: info.fixedIn, file: filePath, category: 'Vulnerable Dependency', owasp: 'A06:2021', source: 'Dependency Audit', recommendation: info.fixedIn ? `Upgrade ${name} to ${info.fixedIn} or later` : `Remove ${name} — no safe version exists.` });
    }
    if (SUSPICIOUS_PATTERNS.some(p => name.toLowerCase().includes(p))) {
      vulnerabilities.push({ package: name, severity: 'MEDIUM', cve: 'N/A', description: `Package name "${name}" suggests dangerous functionality — review carefully`, file: filePath, category: 'Suspicious Dependency', owasp: 'A06:2021', source: 'Dependency Audit', recommendation: 'Audit this package for security issues' });
    }
  }
  return { file: filePath, totalDependencies: Object.keys(allDeps || {}).length, vulnerablePackages: vulnerabilities.length, bySeverity: { CRITICAL: vulnerabilities.filter(v => v.severity === 'CRITICAL').length, HIGH: vulnerabilities.filter(v => v.severity === 'HIGH').length, MEDIUM: vulnerabilities.filter(v => v.severity === 'MEDIUM').length }, vulnerabilities };
}

function auditRequirementsTxt(content, filePath) {
  const vulnerabilities = [];
  ...
  for (const line of lines) {
    ...
    if (info) {
      vulnerabilities.push({ package: pkgName, severity: info.severity, cve: info.cve, description: info.description, fixedIn: info.fixedIn, file: filePath, category: 'Vulnerable Dependency', owasp: 'A06:2021', source: 'Dependency Audit', recommendation: info.fixedIn ? `Upgrade to ${info.fixedIn}+` : 'Remove — no safe version' });
    }
  }
  return { file: filePath, vulnerablePackages: vulnerabilities.length, vulnerabilities };
}

module.exports = { auditPackageJson, auditRequirementsTxt, KNOWN_VULNERABLE_PACKAGES };
